...
func TestCESCacheState(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(*testing.T) {
			...
			if tc.cid.String() == "" {
				cmap.addCEP(CEPName(tc.cepName), tc.cesName, tc.nodeName, tc.gidLabels)
			} else {
			...
			if tc.cid.String() == "" {
				assert.False(t, cmap.hasCID(tc.cid, tc.gidLabels), "CEP with empty CID, so CID should NOT be present in CES cache")
			} else {
				assert.True(t, cmap.hasCID(tc.cid, tc.gidLabels), "CEP with non-empty CID should be present in CES cache")
			}
