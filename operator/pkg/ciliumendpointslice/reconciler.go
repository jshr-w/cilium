...
func (r *slimReconciler) getEndpointEncryptionKey(pod *slim_corev1.Pod) (int, error) {
	switch {
	case r.wgEnabled:
		return int(wgtypes.StaticEncryptKey), nil
	case r.ipsecEnabled:
		node, exists, err := r.ciliumNodeStore.GetByKey(resource.Key{Name: pod.Spec.NodeName})
		if err != nil || !exists {
			return 0, fmt.Errorf("failed to get cilium node for pod %s/%s: %w", pod.Namespace, pod.Name, err)
		}
		return node.Spec.Encryption.Key, nil
	default:
		return 0, nil
	}
}
