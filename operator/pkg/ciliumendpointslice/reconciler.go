...
// We also use the cached encryption key.
...
func (r *slimReconciler) getEndpointEncryptionKey(pod *slim_corev1.Pod) (int, error) {
	key, exists := r.manager.mapping.getEncryptionKey(NodeName(pod.Spec.NodeName))
	if !exists {
		return 0, fmt.Errorf("failed to get encryption key for pod %s/%s", pod.Namespace, pod.Name)
	}
	return int(key), nil
}
