...
func (lw *ciliumEndpointsListerWatcher) getListerWatcher() cache.ListerWatcher {
	lw.once.Do(func() {
		if option.Config.EnableCiliumEndpointSlice {
			...
			lw.sourceObj = &types.CiliumEndpoint{}
		}
	})
	return lw.cachedListerWatcher
}
...
func TransformToCiliumEndpointSlice(obj any) (any, error) {
	...
	case *cilium_api_v2.CiliumEndpoint:
		return ConvertCiliumEndpointToCiliumEndpointSlice(obj)
	case cache.DeletedFinalStateUnknown:
		return ConvertCiliumEndpointToCiliumEndpointSlice(obj)
	default:
		return nil, fmt.Errorf("%T not a known CiliumEndpointSlice object", obj)
	}
}
